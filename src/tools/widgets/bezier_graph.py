...
def _frequency_amplitude_implementation():
    from src.dialog.perlin_noise_transform.graph import FrequencyAmplitudeGraph

    return FrequencyAmplitudeGraph


def _dropoff_implementation():
    from src.dialog.perlin_noise_transform.graph import DropoffGraph

    return DropoffGraph


class BezierCurveGraph:
    """Shared lazy entry point for the application's frequency/amplitude graph."""

    def __new__(cls, *args, **kwargs):
        return _frequency_amplitude_implementation()(*args, **kwargs)

    @staticmethod
    def _normalized_curve_points(points):
        return _frequency_amplitude_implementation()._normalized_curve_points(points)


class DropoffGraph:
    """Shared lazy entry point for the application's drop-off curve editor."""

    def __new__(cls, *args, **kwargs):
        return _dropoff_implementation()(*args, **kwargs)

    @staticmethod
    def _normalized_curve_points(points):
        return _dropoff_implementation()._normalized_curve_points(points)


def evaluate_bezier(points, handles, progress):
    """Evaluate a normalized connected cubic Bezier curve without creating a widget."""
    return evaluate_prepared_bezier(
        prepare_bezier(points, handles),
        progress,
    )
...
def evaluate_prepared_bezier_array(prepared, progress):
    ...
    if len(normalized) < 2:
        return np.full_like(
            values,
            normalized[0][1] if normalized else 0.0,
        )
