...
from src.engine.block_objects import GeneratedMeshBlockObject

from .mesh_object import MeshObject


class GeneratedMesh(MeshObject):
    """Mesh object with an I-by-J-by-K scalar generation field."""
    ...
    def __init__(
        ...
        if block_object is not None and not isinstance(
            ...
            raise TypeError("GeneratedMesh requires a GeneratedMeshBlockObject")
        if grid_data is None and block_object is not None:
            validated_grid_data = block_object.grid_data.copy()
        else:
            validated_grid_data = self._validate_grid_data(grid_data)
        block = block_object or GeneratedMeshBlockObject(
            mesh_data=mesh_data,
    ...
    def grid_data(self, value):
        if hasattr(self, "mesh_block_object"):
            self.mesh_block_object.set_grid_data(value)
