...
from src.components.tree.roots import mesh_root, root_objects, transform_root
from src.dialog.mesh_colourmap import MeshColourmapModel, create_mesh_colourmap_dialog
from src.dialog.mesh_edit.factory import create_mesh_edit_dialog
from src.dialog.mesh_filter import MeshFilterModel, create_mesh_filter_dialog
from src.dialog.mesh_generate import GenerateMeshWindow, MeshGenerateModel
from src.dialog.mesh_import.factory import (
    create_elevation_import_dialog,
    create_mesh_import_dialog,
)
from src.dialog.mesh_procedural.factory import create_mesh_procedural_dialog
from src.dialog.mesh_procedural.model import MeshProceduralModel
from src.dialog.notify import create_notification
from src.engine import EngineTask
...
from src.objects.perlin_noise_transform import PerlinNoiseTransformObject
from src.objects.procedural_mesh import ProceduralMeshObject
from src.tools.dropdown import create_dropdown_menu
...
class MeshImportController:
    ...
    def edit_procedural_mesh(self, mesh_object: ProceduralMeshObject):
        """Open procedural mesh settings and replace the existing procedural mesh."""
        transforms = tuple(
            node.node_object
            for node in transform_root.children
            if isinstance(node.node_object, PerlinNoiseTransformObject)
        )
        window = create_mesh_procedural_dialog(
            model=MeshProceduralModel.from_procedural_mesh(mesh_object),
            on_apply=lambda edited: self._replace_procedural_mesh(mesh_object, edited),
            tree_search=TreeSearch(root_objects.get_nodes()),
            deduper=self._deduper(exclude=mesh_object),
        ...
        if workspace_tabs is None:
            ...
            workspace_tabs.addTab(window, "Edit Procedural Mesh")
            workspace_tabs.setCurrentWidget(window)
        return window
    ...
    def _replace_procedural_mesh(self, mesh_object, edited_mesh):
        was_visible = mesh_object.visible
        colourmap = mesh_object.colourmap
        mesh_object.remove_from_scene()
        mesh_object.mesh_block_object = edited_mesh.mesh_block_object
    ...
    def create_context_menu(self, node, parent: QWidget | None = None):
        ...
        if node is mesh_root:
            ...
            elif isinstance(node.node_object, ProceduralMeshObject):
                options.append(
                    (
                        "Edit Procedural Mesh",
                        lambda: self.edit_procedural_mesh(node.node_object),
                    )
                )
            options.append(
                (
