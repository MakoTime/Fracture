...
class WorldStateObjectTask:
    """Regenerate an existing procedural-mesh block in place."""

    def __init__(self, model, block_object: WorldStateBlockObject):
        self.model = model
        self.block_object = block_object

    def prepare(self):
        load_task = WorldStateTask(self.model, self.block_object)
        return {
            "load": load_task.prepare(),
        }

    def process(self, prepared, progress_callback=None):
        generation_task = WorldStateTask(self.model, self.block_object)
        return generation_task.process(prepared["load"], progress_callback)
