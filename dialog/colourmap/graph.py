...
class ColourmapPreview(QWidget):
    ...
    def __init__(self, positions1=(), positions2=(), colour_grid=(), curve_points1=(), curve_handles1=(), curve_points2=(), curve_handles2=(), parent=None):
        super().__init__(parent)
        self._set_data(positions1, positions2, colour_grid, curve_points1, curve_handles1, curve_points2, curve_handles2)
        self.setMinimumSize(280, 250)

    def set_data(self, positions1, positions2, colour_grid, curve_points1=(), curve_handles1=(), curve_points2=(), curve_handles2=()):
        self._set_data(positions1, positions2, colour_grid, curve_points1, curve_handles1, curve_points2, curve_handles2)
        self.update()

    def _set_data(self, positions1, positions2, colour_grid, curve_points1, curve_handles1, curve_points2, curve_handles2):
        ...
        self.stops = tuple(zip(self.positions1, self.colour_grid[0])) if self.colour_grid else ()
        ...
        if self.colour_grid:
            ...
            for row in range(rows):
                for column in range(columns):
                    ...
                    cell = QRectF(
                        rect.left() + column * rect.width() / columns,
                        rect.top() + row * rect.height() / rows,
                        rect.width() / columns + 1,
                        rect.height() / rows + 1,
                    )
                    ...
                    horizontal = evaluate_prepared_bezier_array(horizontal_curve, horizontal_grid)
                    vertical = evaluate_prepared_bezier_array(vertical_curve, vertical_grid)
                    cell_colours = np.empty((height, width, 4), dtype=float)
                    for channel in range(4):
                        top = (1 - horizontal) * top_left[channel] + horizontal * top_right[channel]
                        bottom = (1 - horizontal) * bottom_left[channel] + horizontal * bottom_right[channel]
                        cell_colours[:, :, channel] = (1 - vertical) * top + vertical * bottom
            ...
            painter.setPen(QPen(self.palette().mid(), 1))
            painter.drawRect(rect)
        painter.drawText(QRectF(rect.left() - 22, rect.bottom() + 7, 44, 18), Qt.AlignmentFlag.AlignHCenter, "0")
        painter.drawText(QRectF(rect.right() - 22, rect.bottom() + 7, 44, 18), Qt.AlignmentFlag.AlignHCenter, "1")
        painter.drawText(QRectF(rect.left(), rect.bottom() + 27, rect.width(), 18), Qt.AlignmentFlag.AlignHCenter, self.field1_name)
        painter.save()
        painter.translate(15, rect.center().y())
        painter.rotate(-90)
        painter.drawText(QRectF(-rect.height() / 2, 0, rect.height(), 18), Qt.AlignmentFlag.AlignHCenter, self.field2_name)
        painter.restore()
        painter.end()
...
class ColourmapAxisGraph(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(QRectF(rect.left() - 25, rect.bottom() + 8, 50, 18), Qt.AlignmentFlag.AlignHCenter, "0")
        painter.drawText(QRectF(rect.right() - 25, rect.bottom() + 8, 50, 18), Qt.AlignmentFlag.AlignHCenter, "1")
        painter.drawText(QRectF(8, rect.bottom() - 9, 32, 18), Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter, "0")
        painter.drawText(QRectF(8, rect.top() - 9, 32, 18), Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter, "1")
        painter.drawText(QRectF(rect.left(), rect.bottom() + 27, rect.width(), 20), Qt.AlignmentFlag.AlignHCenter, self.axis_name)
        painter.save()
        painter.translate(15, rect.center().y())
        painter.rotate(-90)
        painter.drawText(QRectF(-rect.height() / 2, 0, rect.height(), 20), Qt.AlignmentFlag.AlignHCenter, self.axis_name)
        painter.restore()
        if len(self.positions) > 1 and len(self.exponents) == len(self.positions) - 1:
            ...
            for index in range(len(self.positions) - 1):
                ...
                for sample in range(25):
                    ...
                    points.append(self._to_pixel(x, self._segment_value(index, progress)))
    ...
    def mousePressEvent(self, event):
        ...
        distances = [abs(handle.x() - event.position().x()) + abs(handle.y() - event.position().y()) for handle in handles]
    ...
    def mouseMoveEvent(self, event):
        if self._drag_index is None:
            return
        rect = self._plot_rect()
        value = max(0.02, min(0.98, (rect.bottom() - event.position().y()) / rect.height()))
