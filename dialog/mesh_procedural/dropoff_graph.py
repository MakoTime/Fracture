    ...
    def _axis_ticks(minimum, maximum, max_ticks=10):
        ...
        while len(ticks) > max_ticks:
            removable = [
                ...
                if index not in (0, len(ticks) - 1) and abs(tick - midpoint) >= 1e-9
            ]
            if not removable:
                break
    ...
    def paintEvent(self, event):
        ...
        with QPainter(self) as painter:
            ...
            for value in self._axis_ticks(self.frequency_min, self.frequency_max):
                progress = (value - self.frequency_min) / max(
                    0.0001, self.frequency_max - self.frequency_min
                )
                x = rect.left() + progress * rect.width()
                painter.drawLine(
                    QPointF(x, rect.bottom()), QPointF(x, rect.bottom() + 4)
                )
                painter.drawText(
                    QRectF(x - 30, rect.bottom() + 5, 60, 18),
            ...
            if self.curve_mode == "bezier" and self.curve_points:
                path = QPainterPath(
                    self._to_pixel(self.curve_points[0].x(), self.curve_points[0].y())
                )
                ...
                for index, handles in enumerate(self._effective_handles()):
                    ...
                    painter.drawLine(
                        self._to_pixel(anchor.x(), anchor.y()),
                        self._to_pixel(handles[0].x(), handles[0].y()),
                    )
                    painter.drawLine(
                        self._to_pixel(anchor.x(), anchor.y()),
                        self._to_pixel(handles[1].x(), handles[1].y()),
                    )
                return
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.RightButton and self.curve_mode == "bezier":
            ...
            if self._handle_drag_index is None:
                self._handle_drag_index, self._handle_drag_side = self._nearest_handle(
                    event.position()
                )
    ...
    def _nearest_handle(self, position):
        ...
        candidates = [
            (
                ...
                + abs(self._to_pixel(handle.x(), handle.y()).y() - position.y()),
                index,
                side,
            )
        ...
        for index, point in enumerate(points):
            ...
            if handle is None:
                if index == 0:
                    ...
                    vector = QPointF(
                        (right.x() - point.x()) / 3.0, (right.y() - point.y()) / 3.0
                    )
                elif index == len(points) - 1:
                    left = points[index - 1]
                    vector = QPointF(
                        (point.x() - left.x()) / 3.0, (point.y() - left.y()) / 3.0
                    )
                else:
                    left = points[index - 1]
    ...
    def _update_drag(self, position):
        ...
        if self.curve_mode == "bezier" and self.curve_points:
            ...
            self.curve_handles = [
                (
                    QPointF(
                        handle.x() + delta_x, max(0.0, min(1.0, handle.y() + delta_y))
                    ),
                    QPointF(
                        other.x() + delta_x, max(0.0, min(1.0, other.y() + delta_y))
                    ),
                )
            ...
            self._drag_index = min(
                ...
                key=lambda index: (
                    abs(self.curve_points[index].x() - dragged_x)
